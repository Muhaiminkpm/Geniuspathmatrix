...
Your output MUST be a JSON array of 5 career suggestion objects. Do not include any introductory text or conversational filler.`;
  ...
  const prompt = `${systemPrompt}\n\nHere is the student's data: ${JSON.stringify(input)}`;

  try {
    const suggestions = await generateJSON<SuggestCareersOutput>(prompt, SuggestCareersOutputSchema);

    // Concurrently fetch SWOT analysis for each career suggestion
    const suggestionsWithSwot = await Promise.all(
      suggestions.map(async (suggestion: CareerSuggestion) => {
    ...
    console.error("AI failed to return valid career suggestions:", error);
    return [];
  }
}
