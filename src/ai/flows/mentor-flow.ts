...
export async function getSocraticResponse(input: MentorInput): Promise<MentorOutput> {
  const { messages, studentProfile } = input;
...
`;
  ...
  // Convert messages array to a string format for the prompt
  ...
  try {
    ...
    return output ?? "I'm sorry, I'm having trouble thinking of a good question right now. Could you rephrase that?";
  } catch (error) {
    console.error("AI failed to generate mentor response:", error);
    return "I'm sorry, I'm having trouble thinking of a good question right now. Could you rephrase that?";
  }
}
